...
describe("guide loader", () => {
  ...
  it("reports the exact missing matrix entries", async () => {
    await expect(assertCompleteContentMatrix(["en"], root)).rejects.toThrow(/22 missing: en[\\/]guides[\\/]wardogs-playtest\.mdx/i);
  });
  ...
  it("does not count MDX-named directories as files and reports extras", async () => {
    ...
    try {
      ...
      await expect(assertCompleteContentMatrix(["en"], temporaryRoot)).rejects.toThrow(/23 missing:[\s\S]*1 extra: en[\\/]guides[\\/]unlisted\.mdx/i);
    } finally {
