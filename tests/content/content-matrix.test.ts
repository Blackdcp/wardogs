...
describe("English guide library", () => {
  it("contains 47 substantial, unique, manifest-matched guides", async () => {
    ...
    expect(new Set(summaries.map(({title}) => title)).size).toBe(47);
    expect(new Set(summaries.map(({description}) => description)).size).toBe(47);
