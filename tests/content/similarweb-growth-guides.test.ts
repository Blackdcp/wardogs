...
describe("Similarweb growth guide cluster", () => {
      ...
      for (const slug of newGuideSlugs) {
        ...
        const expectedDate = ["wardogs-map", "wardogs-best-settings"].includes(slug)
          ...
          : slug === "wardogs-controls"
            ? "2026-09-04"
            : "2026-08-23";
        expect((guide?.frontmatter.updatedAt ?? "") >= expectedDate).toBe(true);
  ...
  it("upgrades existing ranking pages around the exact player questions", async () => {
    const expectations = new Map([
      ...
      ["wardogs-beta", ["Is WARDOGS Closed Beta 02 live?", "Is the WARDOGS beta free?"]],
    ...
    for (const [slug, phrases] of expectations) {
      ...
      expect((guide?.frontmatter.updatedAt ?? "") >= "2026-08-23").toBe(true);
