...
For a direct Vercel production release, commit the changes and set `INDEXNOW_BASE_SHA` to the previous production revision. Then run `npm.cmd run deploy:prod` in PowerShell. The command refuses a dirty tree or missing base, saves the live sitemap to the ignored `.indexnow/predeploy-sitemap.json` before deployment, deploys, checks the live revision and smoke routes, then compares the new sitemap. It notifies IndexNow of changed live pages plus only URLs newly absent from that snapshot, after verifying each absence or same-site redirect. A failed deployment, smoke check, or notification leaves the snapshot for a same-base-and-HEAD retry; a different release refuses to overwrite it, so inspect and resolve that saved snapshot before proceeding. A successful notification removes it. Check the returned notification count and the public pages; IndexNow submission does not guarantee crawling, indexing, or ranking. The separate GitHub Pages workflow does not use this pre/post-sitemap release path.
